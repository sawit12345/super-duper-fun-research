...
struct NsscSpace {
    ...
    NsscSpace(unsigned n_, unsigned q_)
        : q(q_), l2q(0), N(0), K(0), n(n_), words(n_ >> 6),
          rs(2, 1) {
        assert(n_ % 64 == 0 && (q_ & (q_ - 1)) == 0 && (n_ & (q_ - 1)) == 0);
        unsigned v = q_;
        while (v > 1) { v >>= 1; ++l2q; }
        N = n_ >> l2q;                       // divide-by-pow2 as shift
        this->rs = RsT(N, 1);
    }
    ...
    unsigned to_symbols(const u64* query, u32* sym, unsigned* erase) const {
        unsigned E = 0;
        for (unsigned j = 0; j < N; ++j) {
            // extract q-bit block starting at j*q (q <= 64 assumed)
            u64 blk = extract_block(query, j);
            unsigned pc = (unsigned)__builtin_popcountll(blk);
            if (pc == 1) {
                // symbol = index of set bit within block
                sym[j] = (unsigned)__builtin_ctzll(blk);
            } else {
                sym[j] = 0;
                erase[E++] = j;
            }
    ...
    inline u64 extract_block(const u64* v, unsigned j) const {
        u64 lo = (u64)j << l2q;                         // shift, no multiply
        unsigned wo = (unsigned)(lo >> 6), bo = (unsigned)(lo & 63);
        u64 blk = v[wo] >> bo;
        if (bo + q > 64) blk |= v[wo + 1] << (64 - bo);
        return blk & ((q == 64) ? ~0ull : ((1ull << q) - 1));
    }
