...
int main(int argc, char**) {
    ...
    const unsigned N = 60;                 // NSSC symbols = weight
    const unsigned K = 30;                 // message symbols
    const unsigned n = N * q;              // 3840 bits
    const unsigned w = N;                  // 60
    const unsigned E_STAR = N - K;         // 30 certified flip radius

    printf("# config: n=%u w=%u q=%u N=%u K=%u lambda_max=%u e_star=%u\n",
           n, w, q, N, K, K - 1, E_STAR);
